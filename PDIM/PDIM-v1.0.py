...
Updates:
v0.2:
- Large N of points problem solved:
  condition (if Eti < Et) added: update a point only if the global energy is minimized
v0.3:
- When NaN (not a number) is found, the script stops
  and saves the last minimized codes
v0.4:
- When a point jumps out the surface of the sphere, the script stops
  and saves the last minimized codes
v0.5:
- Bounds applied to limit jacobian matrix results to prevent erros
- Minimal angles added (Mangle)
v0.6:
- Elapsed time function added
v0.7:
- Script rewritten in blocks better defined
v0.8:
- Mirrored minimization added. It is possible to select between two methods:
  1) classical: minimize each point in the system
  2)  mirrored: minimize half of the points in the system, mirroring the other half
- Input parameters method updated to getopt. They are now:
  -p: number of points
  -d: number of dimensions
  -c: number of cycles
  -t: energy gradient tolerance
  -m: minimization method (classical or mirrored)
v:0.9:
- Python 3 compatibility added
v:1.0:
- output parameter defined for printing or not points during minimization
- increment of s by 1 or a power of 2
"""
