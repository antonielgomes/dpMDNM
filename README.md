...
NM-based enhanced-sampling methods highly deppend on the set of NM vectors for an extensive protein conformational exploration. To overcome this, we employed a `uniformly distributed NM vectors`, generatedwe developed a minimization script called [PDIM](https://github.com/antonielgomes/dpMD/tree/main/PDIM), which generates `uniformly distributed NM vectors` that are further utilized to explore the protein conformational space with dpMD.
